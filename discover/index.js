...
module.exports.scan = (ip, range, ports) => {
    if (!isScanning) {
        log.info('Masscan Starting')
        isScanning = true
        ip = ip || '0.0.0.0'
        range = range || '0'
        var mac  = mac || '60-f8-1d-b9-0c-f2'
        var routerMac = routerMac || 'f8-e4-fb-49-df-59'
        ...
        const scanner = spawn('masscan', ['--rate=800','--adapter-mac',mac,'--router-mac',routerMac,'--excludefile',(__dirname + '/exclude.txt'),'-p', ports, (ip + '/' + range)]);
