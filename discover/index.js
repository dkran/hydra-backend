...
module.exports.scan = (ip, range, ports, broadcast) => {
    if (!isScanning) {
        ...
        const scanner = spawn('masscan', ['--rate=400', '-p', ports, (ip + '/' + range)]);
        db.getIPs().then((data) => {
            ...
            discoverService(ip, port, (service) => {
                ...
                if (ip) {
                    db.update(ip, port, service).then((data) => {
                        db.getIPs().then((data) => {
                            log.info('broadcasting initial insert %s', data)
                            broadcast(JSON.stringify({
                                type: 'ipdata', data: data
                            }))
                        })
                    ...
                    db.update(ip, port, service).then((data) => {
                        db.update(record).then((data => {
                            db.getIPs().then((data) => {
                                log.info('broadcasting update %s', data)
                    ...
                    db.getIPs().then((data) => {
                        log.info('broadcasting just generally %s', data)
